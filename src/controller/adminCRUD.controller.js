...
export const AddMovie = async (req, res) => {
    const { movie_name, movie_image, movie_poster, director, category, actors, start_date, duration, language, movie_label, trailer_link } = req.body;

    if (!movie_name || !movie_image || !movie_poster || !director || !category || !actors || !start_date || !duration || !language || !movie_label || !trailer_link) {
        console.log('AddMovie failed: Missing parameter');
        return res.status(400).json({ message: 'Missing parameter' });
    }

    try {
        await addMovie(movie_name, movie_image, movie_poster, director, category, actors, start_date, duration, language, movie_label, trailer_link);
